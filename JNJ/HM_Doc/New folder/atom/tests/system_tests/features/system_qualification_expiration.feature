@system @ALIST-231
Feature: System | Qualification expiration

  Background:
    Given User navigates to Kiosk Administration - System Qualification screen
  ...
  Scenario: An acquisition cannot be run if the System Qualification is expired
    ...
    And User starts an aquisition
    Then User is informed by a popup error message that sample set execution failed
    When User confirms the popup message to open Message Center for details
    Then User confirms that the sample set test has failed due to an expired qualification
